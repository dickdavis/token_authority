...
# Confidential Client OAuth Flow (No PKCE)
#
# This script demonstrates the OAuth flow for a confidential client without PKCE.
# Confidential clients authenticate with client_id and client_secret via HTTP Basic Auth.
#
# Note: While PKCE is optional for confidential clients, it is recommended for
# additional security. See confidential_client_pkce.rb for PKCE version.
#
# Prerequisites:
...
#
# Usage:
#   rails runner script/confidential_client.rb
...
section "Confidential Client OAuth Flow (No PKCE)"
...
section "STEP 1: Authorization Request"

puts "For confidential clients, minimal parameters are required."
puts "The client_id can even be omitted if using HTTP Basic Auth on token endpoint."
puts
...
puts
puts "Note: client_id, redirect_uri, and PKCE are all optional for confidential clients"
puts
